...
function displayRecipeDetails(recipe) {
    ...
    // Show edit/delete/image upload for user-added recipes
    setupUserRecipeControls(recipe);
...
// ============================================
// USER RECIPE CONTROLS (Edit, Delete, Image)
// ============================================

function setupUserRecipeControls(recipe) {
    const editBtn = document.getElementById('editRecipeBtn');
    const deleteBtn = document.getElementById('deleteRecipeBtn');

    if (!recipe.user_added) return;

    // Show edit and delete buttons
    if (editBtn) editBtn.style.display = 'inline-block';
    if (deleteBtn) deleteBtn.style.display = 'inline-block';

    // Edit button — redirect to add-recipe page with recipe ID
    if (editBtn) {
        editBtn.addEventListener('click', () => {
            window.location.href = `add-recipe.html?edit=${recipe.id}`;
        });
    }
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/recipes/delete/${recipe.id}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
                ...
                // Clear localStorage ratings for this recipe
                localStorage.removeItem(`rating_${recipe.id}`);
