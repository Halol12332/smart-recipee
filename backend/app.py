...
- When creating or suggesting a new recipe, ALWAYS use this exact format:
  Recipe Name: [name]
  Ingredients:
...
def add_recipe():
    try:
        ...
        # Validate required fields
        required = ['name', 'cuisine', 'dietary', 'difficulty',
                    'prepTime', 'servings', 'ingredients', 'instructions']
        ...
        data = json.load(open(RECIPES_FILE, 'r', encoding='utf-8'))
        existing = data['recipes']

        # Generate new ID
        max_id = max(int(r['id']) for r in existing) if existing else 0
        ...
        new_recipe = {
            ...
            'nutrition': body.get('nutrition', None),
        ...
        data['recipes'] = existing

        with open(RECIPES_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

        # Reload recipes in memory
        global recipes
        recipes = load_recipes()
...
# ============================================================
# EDIT, DELETE AND IMAGE UPLOAD ENDPOINTS
# ============================================================

@app.route('/api/recipes/edit/<recipe_id>', methods=['PUT'])
def edit_recipe(recipe_id):
    try:
        ...
        recipe_index = next((i for i, r in enumerate(existing)
                             if r['id'] == recipe_id), None)

        if recipe_index is None:
            return jsonify({'success': False,
                            'error': f'Recipe {recipe_id} not found'}), 404

        recipe = existing[recipe_index]

        if not recipe.get('user_added'):
            return jsonify({'success': False,
                            'error': 'Only user-added recipes can be edited'}), 403

        # Update fields
        updatable = ['name', 'cuisine', 'dietary', 'difficulty',
                     'prepTime', 'servings', 'nutrition',
                     'ingredients', 'instructions']
        for field in updatable:
            if field in body:
                recipe[field] = body[field]

        existing[recipe_index] = recipe
        data['recipes'] = existing
...
def delete_recipe(recipe_id):
    try:
        ...
        recipe = next((r for r in existing if r['id'] == recipe_id), None)
        ...
        if not recipe.get('user_added'):
            return jsonify({'success': False,
                            'error': 'Only user-added recipes can be deleted'}), 403

        data['recipes'] = [r for r in existing if r['id'] != recipe_id]

        with open(RECIPES_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

        # Also delete ratings for this recipe
        ratings_data = load_ratings()
        if recipe_id in ratings_data['ratings']:
            del ratings_data['ratings'][recipe_id]
        ...
        return jsonify({'success': True,
                        'data': {'message': f'Recipe {recipe_id} deleted'}}), 200

    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/recipes/upload-image/<recipe_id>', methods=['POST'])
def upload_recipe_image(recipe_id):
    try:
        ...
        if not _allowed_image(file.filename):
            return jsonify({'success': False,
                            'error': 'Only jpg, jpeg, png, webp allowed'}), 400

        # Save to frontend images folder
        _, ext = os.path.splitext(file.filename.lower())
        filename = f"{recipe_id}-custom{ext}"
        images_folder = os.path.join(
            os.path.dirname(__file__), '..', 'frontend', 'images', 'recipes'
        )
        os.makedirs(images_folder, exist_ok=True)
        save_path = os.path.join(images_folder, filename)
        file.save(save_path)

        # Update recipe image path in recipes.json
        image_path = f'./images/recipes/{filename}'
        data = json.load(open(RECIPES_FILE, 'r', encoding='utf-8'))
        existing = data['recipes']
        ...
        if recipe_index is not None:
            existing[recipe_index]['image'] = image_path
            data['recipes'] = existing
            with open(RECIPES_FILE, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
        ...
        return jsonify({'success': True,
                        'data': {'image_path': image_path}}), 200
