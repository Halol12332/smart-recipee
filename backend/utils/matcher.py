...
def normalize_ingredient(ingredient_name):
    ...
    # --------------------------------------------------------
    # DE-PLURALIZATION
    # Handles common plural forms typed by users manually
    # e.g. "eggs" → "egg", "onions" → "onion"
    # --------------------------------------------------------
    plural_rules = {
        # Common ingredients users type in plural
        'eggs': 'egg',
        'onions': 'onion',
        'tomatoes': 'tomato',
        'potatoes': 'potato',
    ...
    # Apply plural rules first
    if ingredient in plural_rules:
        ingredient = plural_rules[ingredient]

    # Generic fallback — remove trailing 's' for unknown plurals
    # Only applies if word is longer than 4 chars to avoid over-stripping
    elif ingredient.endswith('s') and len(ingredient) > 4:
...
# ============================================================
# MATCHER 5: JACCARD SIMILARITY
# ============================================================

def calculate_match_jaccard(user_ingredients, recipe_ingredients):
    """
    Ingredient matching using Jaccard Similarity.
    
    Jaccard Similarity = |A ∩ B| / |A ∪ B|
    
    Unlike normalized matching which only considers recipe ingredients
    as the denominator, Jaccard penalises both missing recipe ingredients
    AND excess user ingredients that are not needed.
    
    Reference: Jaccard, P. (1912). The distribution of flora in the
    alpine zone. New Phytologist, 11(2), 37-50.
    """
    user_set = set(normalize_ingredient(ing) for ing in user_ingredients)
    recipe_set = set(normalize_ingredient(ing['name']) for ing in recipe_ingredients)
    ...
    match_percentage = jaccard_score * 100
    ...
    return {
        ...
        'missing_ingredients': list(missing),
        'total_required': len(recipe_set),
        'total_matched': len(intersection)
    }
...
def get_recipe_recommendations(user_ingredients, recipes, min_match=0, method='normalized'):
    ...
    matchers = {
        ...
        'jaccard': calculate_match_jaccard
    }
    ...
    for recipe in recipes:
        ...
        if match_info['match_percentage'] >= min_match:
            recommendation = {
                ...
                'user_added': recipe.get('user_added', False)
            }
